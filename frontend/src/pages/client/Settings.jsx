...
export default function ClientSettings() {
  ...
  return (
    <DashboardLayout>
      <div className="border-b-4 border-double border-ink mb-8 pb-4"><h1 className="font-serif text-3xl font-black text-ink">Settings</h1></div>
        ...
        <div className="bg-parchment border border-rule rounded-lg p-6">
          ...
          <div className="space-y-3">
            {[["Name",user?.name],["Email",user?.email],["Role",user?.role]].map(([l,v])=>(
              <div key={l} className="flex justify-between py-2 border-b border-rule/50 last:border-b-0"><span className="text-xs font-sans text-ink-muted">{l}</span><span className="text-sm font-sans font-semibold text-ink capitalize">{v}</span></div>
            ))}
          </div>
        </div>
        <div className="bg-parchment border border-rule rounded-lg p-6">
          ...
          <form onSubmit={handleChange} className="space-y-3">
            ...
            <div><label className="block text-xs font-sans font-semibold text-ink mb-1">Confirm</label><input type="password" className={inputClass} value={pw.confirmPassword} onChange={e=>setPw({...pw,confirmPassword:e.target.value})} required/></div>
            <button type="submit" className="w-full bg-accent hover:bg-accent-dark text-white py-2.5 rounded-lg font-sans text-sm font-semibold disabled:opacity-50" disabled={loading}>{loading?"...":"Change Password"}</button>
          </form>
        </div>
