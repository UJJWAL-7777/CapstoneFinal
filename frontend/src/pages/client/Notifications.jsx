      ...
      : notifications.length > 0 ? (
        <div className="space-y-2">{notifications.map(n=>(
          <div key={n._id} onClick={()=>{if(!n.isRead)markRead(n._id);if(n.link)navigate(n.link);}}
            ...
            <div className="flex-1 min-w-0"><strong className="font-sans text-sm font-semibold text-ink block">{n.title}</strong><p className="text-xs font-sans text-ink-muted mt-0.5">{n.message}</p><span className="text-[10px] font-sans text-ink-faint">{new Date(n.createdAt).toLocaleString()}</span></div>
