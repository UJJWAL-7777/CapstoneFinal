...
const TABS = ["all","active","pending","in_progress","resolved","closed"];
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex gap-1 border-b-2 border-rule mb-5 overflow-x-auto">
        {TABS.map(t=>(
          <button key={t} onClick={()=>setTab(t)}
            className={`px-4 py-2.5 text-sm font-sans font-medium capitalize whitespace-nowrap border-b-2 -mb-[2px] transition-colors ${tab===t?"text-accent border-accent font-semibold":"text-ink-muted border-transparent hover:text-ink"}`}>
            {t.replace("_"," ")}
          </button>
        ))}
      </div>
      ...
      <div className="bg-parchment border border-rule rounded-lg p-5">
        {loading ? <p className="text-ink-muted text-sm font-sans text-center py-6">Loading cases...</p>
        : cases.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm font-sans">
              <thead><tr className="border-b-2 border-rule">
                {["Case #","Title","Client","Type","Status","Created",""].map(h=><th key={h} className="text-left py-2.5 px-3 text-ink-muted text-xs font-semibold uppercase tracking-wider">{h}</th>)}
              </tr></thead>
              <tbody>{cases.map(c=>(
                <tr key={c._id} className="border-b border-rule/50 hover:bg-cream/50 transition-colors">
                  ...
                  <td className="py-2.5 px-3"><Link to={`/advocate/cases/${c._id}`} className="text-accent text-xs font-semibold hover:underline">View</Link></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
        ) : <p className="text-ink-faint text-sm font-sans italic text-center py-6">No cases found for this filter.</p>}
      </div>
    </DashboardLayout>
  );
}
