  ...
  return (
    <DashboardLayout>
      ...
      <div className="bg-parchment border border-rule rounded-lg px-5 py-3 flex gap-3 flex-wrap items-center mb-5">
        <select value={filters.caseType} onChange={e=>setFilters({...filters,caseType:e.target.value})} className={inputClass}><option value="">All Types</option>{TYPES.filter(Boolean).map(t=><option key={t} value={t}>{t}</option>)}</select>
      ...
      <div className="bg-parchment border border-rule rounded-lg p-5">
        ...
        : cases.length > 0 ? (
          <div className="overflow-x-auto"><table className="w-full text-sm font-sans">
            <thead><tr className="border-b-2 border-rule">{["Case #","Title","Advocate","Type","Outcome","Closed"].map(h=><th key={h} className="text-left py-2.5 px-3 text-ink-muted text-xs font-semibold uppercase tracking-wider">{h}</th>)}</tr></thead>
            <tbody>{cases.map(c=>(
              <tr key={c._id} className="border-b border-rule/50 hover:bg-cream/50"><td className="py-2.5 px-3 font-mono text-xs text-ink-muted">{c.caseNumber}</td><td className="py-2.5 px-3">{c.title}</td><td className="py-2.5 px-3">{c.advocate?.name||"—"}</td><td className="py-2.5 px-3">{c.caseType}</td><td className="py-2.5 px-3">{c.outcome||"—"}</td><td className="py-2.5 px-3 text-ink-faint text-xs">{new Date(c.updatedAt).toLocaleDateString()}</td></tr>
            ))}</tbody>
          </table></div>
        ) : <p className="text-ink-faint text-sm font-sans italic text-center py-6">No resolved or closed cases.</p>}
      </div>
    </DashboardLayout>
  );
}
